...
class ChangeTextTest {
    ...
    fun testEmptyField() {
        ...
        assertEquals(startText, result1)
        // ввод пробелов в поле
        device.findObject(By.res(packageName, "userInput")).text = "   "
        device.findObject(By.res(packageName, "buttonChange")).click()

        val result2 = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(startText, result2)
    }
    ...
    fun testButtonActivity() {
        ...
        device.wait(Until.findObject(By.res(packageName, "text")), TIMEOUT)
        //waitForPackage("text")
